...
from sage_prot.utils.goal_directed_score_contributions import ScoreContributionSpecification, compute_global_score
from sage_prot.scoring.scoring_function import ScoringFunction, ScoringFunctionWrapper
from sage_prot.utils.goal_directed_generator import GoalDirectedGenerator
from sage_prot.scoring.filters import clean_sequence, clean_sequence_list
...
class GoalDirectedBenchmark:
    ...
    def assess_model(self, model: GoalDirectedGenerator) -> GoalDirectedBenchmarkResult:
        ...
        molecules = model.generate_optimized_molecules(scoring_function=self.objective,
                                                       number_molecules=number_molecules_to_generate,
                                                       starting_population=self.starting_population
                                                       )
        end_time = time.time()

        clean_molecules = clean_sequence_list(molecules)
        unique_molecules = remove_duplicates(clean_molecules)
