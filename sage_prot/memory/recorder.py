...
def unravel_elements(elements: List[RecorderElement]) -> Tuple[List[str], List[float]]:
    return tuple(  # type: ignore
        map(list, zip(*[(element.sequence, element.score) for element in elements]))
    )
...
class Recorder:
    ...
    def add_list(self, sequences: List[str], scores: List[float]) -> None:
        new_elements = [
            RecorderElement(sequence=sequence, score=score)
            for sequence, score in zip(sequences, scores)
        ]
        new_elements = list(set(new_elements))
        new_elements = list(
            filter(lambda element: element.sequence not in self.seen_sequences, new_elements)
        )
        self.seen_sequences = self.seen_sequences.union(sequences)
        ...
        if self.record_filtered:
            ...
            filtered_new_elements = list(
                filter(
                    lambda element: self.rd_filter(element.sequence) > 0.5,
                    filtered_new_elements,
                )
            )
    ...
    def log_final(self) -> None:
        for element in self.elements:
            self.logger.log_text("optimized_sequence", element.sequence)
            self.logger.log_metric("optimized_score", element.score)

        if self.record_filtered:
            for element in self.filtered_elements:
                self.logger.log_text("filtered_sequence", element.sequence)
    ...
    def get_topk(self, top_k: int):
        ...
        return [element.sequence for element in self.elements], [
