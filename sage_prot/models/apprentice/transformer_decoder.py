...
from sage_prot.models.apprentice import AbstractGenerator

class PositionalEncoding(nn.Module):
    def __init__(self, n_embed: int, dropout: float = 0.2, max_seq_len=512) -> None:
        super(PositionalEncoding, self).__init__()
        self.dropout = nn.Dropout(p=dropout)

        pe = torch.zeros(max_seq_len+8, n_embed)
        position = torch.arange(0, max_seq_len+8, dtype=torch.float).unsqueeze(1)
        div_term = torch.exp(
