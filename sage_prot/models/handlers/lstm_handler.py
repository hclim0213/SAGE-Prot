...
from sage_prot.models.apprentice import (
    AbstractGenerator,
    LSTMGenerator,
)
from sage_prot.models.handlers.abstract_handler import AbstractGeneratorHandler
...
class LSTMGeneratorHandler(AbstractGeneratorHandler):
    ...
    def train_on_batch(self, sequences: List[str], device: torch.device) -> float:
        actions, _ = sequences_to_actions(self.char_dict, sequences)
    ...
    def sample(self, num_samples: int, device: torch.device, **kwargs):
        ...
        sequences = self.char_dict.matrix_to_sequences(action, seq_length - 1)
        return sequences, action, log_prob, seq_length
